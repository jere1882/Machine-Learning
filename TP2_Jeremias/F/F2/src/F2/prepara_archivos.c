...
int create_files(char *filename, int class){
  ...
  /* Creamos el .data */
  sprintf(filepat,"%s.%d.data",filename,class);
  fdata=fopen(filepat,"w");
  error=(fdata==NULL);
  if(error){
    printf("Error creando el %s.%d.data \n",filename,class);
    return 1;
  }
  
  int i,k;

  /* Modificar sus clases a o-1*/
  for(k=0; k < PTOT ; k++){
  ...
  /* Creamos el .test */
  sprintf(filepat,"%s.%d.test",filename,class);
  ftest=fopen(filepat,"w");
  error=(ftest==NULL);
  if(error){
    printf("Error creando el %s.%d.test \n",filename,class);
    return 1;
  }
