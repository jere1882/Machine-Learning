...
int define_matrix(){
  ...
  votaciones = (int*) calloc(N_Class,sizeof(int));
...
double distancia_euclideana(double *fp, double *fx) {
	double x,sum = 0.0;
	int i;
	for (i=0; i<N_IN ; i++)
           sum += pow(fp[i]-fx[i],2);
...
int output(double *input){
  int clase=-1,pclase=-1;
  int k;
  double *x;

  for(k=0;k<PR;k++){
    x = data[seq[k]];
    if(distancia_euclideana(input,x) <= K) 
      votaciones[(int)x[N_IN]]++;
  }
 
  for(k=0;k<N_Class;k++) {
    if (pclase<votaciones[k]){
	pclase=votaciones[k];
        clase = k;
    }
  }
...
/* ese offset indica vale 1 si estoy clasificando training y 0 si estoy clasificando puntos de validación o test)*/
