...
int K;              /*Número de vecinos (parámetro del algoritmo K-nn)*/
...
double **distancias;               /* mismas dimensiones que data. se usará para calcular distancias*/ 
...
int define_matrix(){
  ...
  distancias=(double **)calloc(PTOT,sizeof(double *)); 
  ...
  for(i=0;i<PTOT;i++)
    distancias[i]=(double *)calloc(N_IN+1,sizeof(double));
...
int arquitec(char *filename){
  ...
  printf("\nCantida de vecinos a considerar: %d\n",K); 
...
double dist(const void *p, void *x) {
	double *fp,*fx,sum;
	fp = *(double **)p;
        fx = (double*) x;
	int i;
...
/* compara dos puntos de acuerdo a su distancia a un tercero
   se usará para ordenar con quicksort */
int cmp (const void *p1, const void *p2, void *x){
    double d1 = dist (p1,x);
    double d2 = dist (p2,x);
    if (d1<d2)       return -1;
...
/* ------------------------------------------------------------------------------ */
int output(double *input,int offset){
/*el nuevo argumento indica si el punto a clasificar es o no de training */

  double prob_de_clase;
  double max_prob=-1e40;
  int i,j,k,clase_MAP;

  /* Copiamos data a distancias */
  for (i=0; i<PR ; i++)
      for (j=0; j<N_IN+1 ; j++)
	 distancias[i][j]=data[seq[i]][j];

  /* Ordenamos el data de acerdo a la distancia a input */
  qsort_r(distancias,PR,sizeof(double *), cmp, (void *)input);

  for(k=0;k<N_Class;k++) {

    prob_de_clase=0.;

    for(i=0+offset;i<K+offset;i++) prob_de_clase += distancias[i][N_IN]==k ? 1 : 0; 
    /*Si es de taining, arranco de 1 para que no se vote a si mismo!*/
...
/* ------------------------------------------------------------------------------ */
double propagar(double **S,int pat_ini,int pat_fin,int usar_seq,int offset){
/* ese offset es 1 si estoy clasificando training y 0 si estoy clasificando puntos de validación o test)*/

  //printf ("Iniciando propagación.\n");
  double mse=0.0;
  int nu;
  int patron;
  ...
  for (patron=pat_ini; patron < pat_fin; patron ++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    //printf ("Estamos clasificando el input %d\n",nu);
    pred[nu]=output(S[nu],offset);
...
int train(char *filename){
 // printf ("Comenzando el training \n");
  ...
  else         valid_error=propagar(data,PR,PTOT,1,0);
  /*calcular error de test (si hay)*/
  if (PTEST>0) test_error =propagar(test,0,PTEST,0,0);
  else         test_error =0.;
  /*mostrar errores*/
