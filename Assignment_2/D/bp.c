...
int arquitec(char *filename){
  ...
  /* AGREGADO: Leemos un nuevo parámetro del .net */
  fscanf(b,"%f",&GAMMA);
...
float propagar(float **S,int pat_ini,int pat_fin,int usar_seq){
  ...
  float mse=0.0;
  float w=0.0; // AGREGADO: TÉRMINO EXTRA QUE SUMAREMOS AL ERROR
  int patron,nu;
  discrete_error=0.;
 ...
 /* AGREGADO: Calculamos el término extra del error */
 
 // penalización de la primera capa 
   for(i=1;i<=N2; j++)
      for(j=0;j<=N1;j++)
          w +=pow(w1[i][j],2);
          
 //penalización de la segunda capa      
  for(i=1;i<=N3;i++)                    
      for(j=0; j<=N2; j++)
          w += pow(w2[i][j],2);

  /* AGREGADO: Sumamos el término extra al error */
  mse+= w* GAMMA; 
...
int train(char *filename){
  ...
  /* Matrices creadas con calloc, ya estan inicializadas en cero */
  for (j = 1; j <= N2; j++) {
    grad2[j] = 0.0;
  ...
  for (iter=1; iter<=ITER; iter++) {
    ...
    for(nu1=0;nu1<PR;nu1++) {
	  ...
	  for( i = 1 ;i<= N3;i++) for( j = 0 ;j<= N2;j++){
        ...
        /* AGREGADO: Uso la nueva weight update rule indicada en el libro */
		w2[i][j] = dw2[i][j] + w2[i][j]*(1 - 2.0 * eta * GAMMA);
	  ...
	  for( j = 1 ;j<= N2;j++) for( k = 0 ;k<= N1;k++){
        ...
        /* AGREGADO: Uso la nueva weight update rule indicada en el libro */
		w1[j][k] = dw1[j][k] + w1[j][k]*(1 - 2.0 * eta * GAMMA);
    ...
    if ((iter/NERROR)*NERROR == iter) {
      ...
      /* Para ir agregándolo en el .mse, vuelvo a calcular el término añadido al error */
 float w=0;
 // penalización de la primera capa 
   for(i=1;i<=N2; j++)
      for(j=0;j<=N1;j++)
 ...
 //penalización de la segunda capa      
  for(i=1;i<=N3;i++)                    
      ...
      fprintf(ferror,"%f\t%f\t%f\t%f\n",disc_train,disc_valid,disc_test,w);
