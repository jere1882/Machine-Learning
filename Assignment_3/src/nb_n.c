...
double **mu;                       /* mu de cada componente de cada clase */
double **var;                    /* var de cada componente de cada clase */
...
int define_matrix(){
  ...
  mu = (double**) calloc(N_Class,sizeof(double *));
  for (i=0; i<N_Class; i++)
	mu[i] = (double *) calloc (N_IN, sizeof(double));
	
  var = (double**) calloc(N_Class,sizeof(double *));
  for (i=0; i<N_Class; i++)
	var[i] = (double *) calloc (N_IN, sizeof(double));
...
double normal(double mu, double varianza, double x) {
	double denom = sqrt(2 * varianza * M_PI);
	double num   = exp(-pow((x-mu),2)/(2*varianza));
	return (num/denom);
}
...
double prob(double x,int feature,int clase)  { 
	return normal(mu[clase][feature],var[clase][feature],x);
}
...
int train(char *filename){
  ...
  /*Calcular cuantos elementos hay por clase */
	for (i=0; i < N_TOTAL ; i++)
		nejs[(int)data[i][N_IN]] += 1;
		
  /*Calcular probabilidad intrinseca de cada clase*/
	for (i=0 ; i < N_Class ; i++)
		p_clase[i] = (float) nejs[i] / (float) N_TOTAL;

  /*Calcular media y desv.est. por clase y cada atributo*/


	for (j=0 ; j< N_IN ; j++){  // para cada componente
		for (i=0; i<N_TOTAL; i++) // para cada punto
			mu[(int)data[i][N_IN]][j]+= data[i][j]; // sumo los valores de la j-ésima componente en la clase correspondiente
		for (i=0; i<N_Class; i++)
			mu[i][j] /= nejs[i]; // divido por el total de puntos de esa clase 
	}
	...
	for (j=0 ; j< N_IN ; j++){  // para cada componente
		for (i=0; i<N_TOTAL; i++){ // para cada punto
			c = (int) data[i][N_IN];
			dif = data[i][j] - mu[c][j];
			var[c][j]+= dif * dif; // sumo la diferencia cuadradas de las j-ésima componente en la clase correspondiente
		}
		for (i=0; i<N_Class; i++)
			var[i][j] /= nejs[i]; // divido por el total de puntos de esa clase
	}
