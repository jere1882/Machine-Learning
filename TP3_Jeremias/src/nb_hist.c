...
double **min_cubeta;                /* límite inferior de cada grupo de cubetas */
double **max_cubeta;                /* límite superior de cada grupo de cubetas */
...
int define_matrix(){
  ...
  p_clase = (double *) calloc(N_Class,sizeof(double));
	...
	for(i = 0; i < N_Class; i++) {
		bins[i] = (double**)calloc(N_IN, sizeof(double*));
		for(j = 0; j < N_IN; j++) {
  ...
  min_cubeta = (double**) calloc (N_Class,sizeof(double*));
  max_cubeta = (double**) calloc (N_Class,sizeof(double*));
  for (i=0; i<N_Class; i++){
    min_cubeta[i]=(double*) calloc (N_IN, sizeof(double));  
    max_cubeta[i]=(double*) calloc (N_IN, sizeof(double));  
  }
...
int arquitec(char *filename){
  ...
  printf("\nCantidad de bins por clase por feature: %d",N_BINS);
...
int hallar_cubeta(double x, int feature, int clase){ // A qué número de cubeta corresponde x?
	double ancho_cubeta = (max_cubeta[clase][feature]-min_cubeta[clase][feature])/(double)N_BINS;
	if (ancho_cubeta<0) ancho_cubeta*=-1;
	int i;
	for (i=0; i<N_BINS; i++){
       if (x <= min_cubeta[clase][feature] + (i+1)*ancho_cubeta) return i;
	}
	return (N_BINS-1)
}
...
double prob(double x,int feature,int clase)  { 
	int c = hallar_cubeta(x,feature,clase);
	return bins[clase][feature][c];
}
...
int train(char *filename){
  ...
  /*Calcular probabilidad intrinseca de cada clase*/
    ...
    * para distintas clases. ME fijo para cada clase límites adecuados */
    ...
    float x;
    int class;
    /* Primero hallo neutros adecuados para poder hallar el minimo y maximo real de cada feature de cada clase */
    double min,max;
    for (j=0 ; j < N_IN ; j++){
		min = max =data[seq[0]][j];
		for (k=1 ; k < N_TOTAL ; k++) {
			if (min > data[seq[k]][j]) min = data[seq[k]][j];
			if (max < data[seq[k]][j]) max = data[seq[k]][j];
		}
		for (i=0 ; i< N_Class ;i++){
			min_cubeta[i][j]=max;
			max_cubeta[i][j]=min;
    ...
    /* Ahora hallo  el mínimo y máximo de cada conjunto de cubetas de acuerdo a su clase (min y max estan inicializados con neutro)*/
	for (j=0 ; j<N_IN ; j++) {
			for (k=0; k< N_TOTAL ; k++) {
				x     = data[seq[k]][j];
				class = (int) data[seq[k]][N_IN];
				if (min_cubeta[class][j] > x)  min_cubeta[class][j]=x;
				if (max_cubeta[class][j] < x)  max_cubeta[class][j]=x;
			}
	}
   ...
   /* Armo los histogramas por cada clase */
	...
	for (j=0 ; j<N_IN ; j++) {
			for (k=0; k< N_TOTAL ; k++) {
				x     = data[seq[k]][j];
				class = (int) data[seq[k]][N_IN];
				cubeta = hallar_cubeta(x,j,class);
				//printf ("(clase %d feature %d) valor %f metido en la cubeta %d \n",class,j,x,cubeta);
				bins[class][j][cubeta]++;
			}
	}
 	...
 	for (j=0 ; j<N_IN ; j++) {
		for (k=0; k< N_Class ; k++) {
			for (i = 0; i<N_BINS; i++){
				bins[k][j][i] +=1;  // sumo p.m
				bins[k][j][i] /= nejs[k] + N_BINS;  // divido por n+m
			}
		}
	}
